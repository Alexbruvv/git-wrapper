...
export async function isGitInstalled(runner: Runner = realRunner): Promise<boolean> {
  try {
...
export async function isInsideWorkTree(runner: Runner = realRunner): Promise<boolean> {
...
export async function getOriginUrl(runner: Runner = realRunner): Promise<string | null> {
  const { code, stdout } = await runner.capture("git", [
    "remote",
    "get-url",
    "origin",
