...
export class Gh {
  ...
  async status(): Promise<Account[]> {
    const json = await this.runner.capture("gh", ["auth", "status", "--json", "hosts"]);
  ...
  async switch(host: string, user: string): Promise<void> {
    ...
    if (code !== 0) {
      throw new GhError(`gh auth switch failed: ${stderr.trim() || `exit ${code}`}`);
    }
  }
  ...
  async setupGit(host: string): Promise<void> {
    ...
    if (code !== 0) {
      throw new GhError(`gh auth setup-git failed: ${stderr.trim() || `exit ${code}`}`);
    }
  }
}
...
export function parseJsonStatus(stdout: string): Account[] {
  ...
  for (const entries of Object.values(data.hosts ?? {})) {
    for (const e of entries) {
      accounts.push({ user: e.login, host: e.host, active: Boolean(e.active) });
    }
  }
  return accounts;
}
