...
export async function doctor(
  ...
  out(`${ghOk ? PASS : FAIL} gh installed${ghOk ? "" : " — see https://cli.github.com"}`);
  ok &&= ghOk;
    ...
    try {
      ...
      if (accounts.length === 0) {
        ...
        for (const a of accounts) {
          out(`    - ${a.user} @ ${a.host}${a.active ? " (active)" : ""}`);
        }
      }
    } catch (err) {
      out(`${FAIL} gh auth status failed: ${err instanceof Error ? err.message : err}`);
      ok = false;
    }
  }
